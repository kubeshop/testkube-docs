...
* `<jtlFile>` - path to the JTL report file
* `<reportFile>` - path to the JMeter dashboard report directory
...
and the JMeter executor is configured to store the JTL report and dashboard report after the test run. You can get
the files from the "Artifacts" tab in the execution results in the Testkube Dashboard, or download them with the
`testkube get artifacts EXECUTION_ID` command.

When the JMeter command includes `-e -o <report-directory>`, JMeter creates a dashboard report that includes
`statistics.json`. If that file is uploaded as an artifact, Testkube can detect it as a `jmeter.statistics` report
and ingest sample counts, error rates, response-time aggregates and percentiles, throughput, and sent/received
throughput as [Granular Metrics](/articles/granular-metrics).
