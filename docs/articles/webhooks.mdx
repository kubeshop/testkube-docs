...
In order to re-use Webhook specification, you can define a special Testkube resource - WebhookTemplate.
It contains all the same fields as Webhook resource, except `webhookTemplateRef` one, which is used for referencing template in webhook.
WebhookTemplate can be used only as part of the calling Webhook. It's content is merged into Webhook specification.
...
In order to send the message when test workflow execution finishes, the following Webhook and Webhook template can be used:
