...
#### Using Agent Environment Variables in Webhooks

In addition to event-specific variables, it's possible to use environment variables from the Agent API server
(on agents with the [Webhook capability](/articles/agents-overview#webhook-agents)) in your webhook payloads and configurations.
...
To set up extra environment variables for the Agent API Server, you can use the `testkube-api.extraEnvVars` field in your Helm chart configuration.
...
- `Namespace` - TestWorkflowExecution namespace (for example, `my-testkube`)
...
- `Reports` - TestWorkflow reports generated from the steps, like junit
      ...
      "spec": {
        "content": {
          "git": {
            ...
            "revision": "main",
            "paths": ["test/pytest/pytest-project"]
          }
        },
        "container": {
          "workingDir": {
...
- List existing: `kubectl get webhooktemplates -n <namespace>`
