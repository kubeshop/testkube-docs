...
<Tabs groupId="install-runner-agent">
...
Testkube Environment, use the `testkube install agent <name> --create --runner` command to install your first Runner Agent:
...
Open the Testkube Dashboard and navigate to the **Agents** page for your Environment (see [Adding Agents to an Environment](/articles/agents-overview#adding-agents-to-an-environment) for more details). Click the **Connect New Agent**
...
1. Choose the **Runner** capability for the new Agent.
2. Select your preferred installation tool - **Testkube CLI**, **Helm**, or **GitOps**.
3. Copy and run the generated commands (or YAML) against the cluster/namespace where you want the Runner Agent to execute Workflows.

Once the Agent connects successfully, it will appear in the list of Agents and is ready to run your Workflows.
