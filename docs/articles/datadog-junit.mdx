...
1. Your test step produces JUnit XML files and saves them under the default working directory (/data/repo) or any specified directory
...
spec:
  ...
  - name: Upload JUnit to Datadog        
    run:
      ...
      - "**/*"                         # glob picking up every JUnit file - this could be paramterized to a specific directory 
...
metadata:
  ...
  namespace: testkube
spec:
  config:                             # This config statement is an optional parameter used by the DataDog CLI to categorize results 
    service:
      type: string
...
...
