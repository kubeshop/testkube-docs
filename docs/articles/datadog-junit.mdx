...
1. Your test step produces JUnit XML files and saves them under the default working directory (/data/repo/) or any directly path specified for the testing tool executing the tests and writing the JUnit output
2. A template step runs the official `datadog/ci` container and calls `datadog-ci junit upload`, passing parameterized metadata such as the service name or working directory
...
- This template is parameterized to accept a “service” name used by the DataDog CLI to align the test results with a DataDog "service" - modify the configuration element with the parameters needed for your command configuration
