...
1. Your test step produces JUnit XML files and saves them under **/data/artifacts** (the default working directory in Testkube TestWorkflow).
2. A template step runs the official `datadog/ci` container and calls `datadog-ci junit upload`, pointing it at the artifacts directory and passing metadata such as the service name.
...
spec:
  ...
  - name: Upload JUnit to Datadog
    workingDir: /data/artifacts        # where Testkube stores saved artifacts
    run:
      ...
      - "**/*"                         # glob picking up every JUnit file
...
## 4 Re‑using the pattern for other CLI integrations
