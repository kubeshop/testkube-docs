...
| `reasoningEffort`  | Thinking level used unless the user picks one — agent and tasks only. See [Thinking Levels](#thinking-levels) |
...
- `reasoningEffort` sets the level the AI service uses when the user does not pick one.
- `reasoningEfforts` lists the levels users may pick. The dashboard shows a thinking level dropdown next to the model dropdown, in AI Chats and in AI Test Authoring, only for models that list one.
  The order in the list is the order in the dropdown. If `reasoningEffort` is not in the list, the first listed level becomes the default.
- Omit `reasoningEfforts` to keep the level fixed at `reasoningEffort`.
...
- Any model with a thinking level configured (`reasoningEffort` or `reasoningEfforts`). Since GPT-5.4, OpenAI only accepts tool calls with a reasoning effort on `/v1/responses`.
