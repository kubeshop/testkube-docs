...
When it is not set, models call Chat Completions, except models that cannot work there, which switch to the Responses API automatically:
...
Pin today's behaviour for every model (recommended before upgrading):
