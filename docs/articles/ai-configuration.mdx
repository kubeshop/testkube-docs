...
In addition to the platform default model, you can add your own LLM providers directly from the Dashboard
...
via [Custom LLM Providers](/articles/ai-models) in the Dashboard.
...
testkube-ai-service:
  ...
  models:
    ...
    - name: gpt-5-mini
      ...
      temperature: 1            # Optional - omit to use the model's default
...
|----------|----------|-------------|
| `name` | Yes | Model identifier (e.g., `gpt-5.2-codex`, `gpt-4o`) |
| `type` | No | `primary` or `lightweight`. First model defaults to `primary` if not specified |
| `temperature` | No | Temperature setting (0-2). Omit to use the model's default. Some models like `gpt-5.2` only support the default value |
| `url` | No | Custom LLM endpoint URL (defaults to global `llmApi.url` or OpenAI) |
| `apiKey` | No | Direct API key value (not recommended for production) |
| `secretRef` | No | Kubernetes secret name containing the API key |
| `secretRefKey` | No | Key within the secret (defaults to `LLM_API_KEY`) |
...
Testkube evaluates model configuration in the following order (highest priority first):

1. **`modelsFile`** — External YAML file (see below)
2. **`models`** — Array of model configurations in Helm values
3. **`model`** — Simple single model name

Only the highest-priority configuration that is set will be used.
...
For deployments that manage configuration outside of Helm values, you can provide a models file directly:
...
This is useful when model configurations are managed by external tooling or stored separately from your Helm values.
...
- [Configuring AI Models](/articles/ai-models) — Adding custom LLM providers from the Dashboard
