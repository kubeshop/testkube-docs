...
In this example, 4 workers are started but worker 1 fails after 2 seconds. With `failFast: true`, the remaining workers are immediately cancelled.
...
Without `failFast` (the default), all workers run to completion even if one fails. This allows you to collect results from all workers.
