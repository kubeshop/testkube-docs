...
- **`sed -E 's#<(failure|error).*##'`** trims each line back to the `<testcase` attributes, so a `name=` inside the failure body cannot be mistaken for the case's own. It cuts at the child element rather than at the first `>`: a raw `>` is legal inside an attribute value, so cutting at `>` would truncate `name="a > b"` mid-value and drop that case from the selection without a word. `<` is _not_ legal there — it has to be `&lt;` — so `<failure` and `<error` only ever appear as the child they look like.
...
Two limits of doing this with text tools rather than an XML parser: a **numeric** character reference (`&#38;`) is not decoded by the `sed` above, and an attribute value spanning a newline inside the tag would not be matched. Both are rare in generated reports; if your suite hits either, parse the report with a real XML parser — `xmllint --xpath` or a few lines of Python — rather than extending the pipeline.
...
Note where that sits: `fetch` belongs to an entry of an `execute` step, so it comes free when the rerun runs its tests as a separate Workflow, and is awkward when it does not — there is no standalone fetch step. Reach for the CLI only if neither `read_artifact()` nor `fetch` fits.
