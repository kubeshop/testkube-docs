...
An [`execute`](/articles/test-workflows-test-suites) entry can schedule a rerun as well, by naming the execution it descends from:
...
- It runs the Workflow's **current definition**, not the base's stored snapshot — the equivalent of `--latest`.
- The base must be an execution the scheduling run may itself read: its own, its parent, one of its children, a sibling from the same parent, or its own base. Naming one it could not otherwise reach is refused, so this cannot be used to read an execution through a child.
...
`execution.lineage` is resolved **in the pod**, when the step runs — unlike `execution.id` and the rest of `execution.*`, which are substituted while the Workflow is being prepared. That is deliberate: a rerun replays the definition as it was resolved for the execution it descends from, so a lineage substituted at preparation time would be the _original's_ lineage, and every rerun would believe it was an original run. Step `condition` is resolved in the pod as well, which is what lets one branch on lineage — as the example below does to keep its rerun-only step out of an original run's way.
