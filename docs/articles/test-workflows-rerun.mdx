...
- [`fetch`](/articles/test-workflows-execution-sharing#downloading-artifacts-to-disk) **cannot pre-load a file for a run.** It belongs to an entry of an `execute` step and runs **after** that entry's executions finish, writing onto the pod that scheduled them. So `fetch` collects a report once the tests are over; it cannot hand one to tests that have not started, and a child Workflow never sees it.
- **`testkube download artifact` in the step** does work, and is what the [Playwright example](/articles/examples/playwright-rerun) does. The cost is that the CLI needs an API token in the Workflow, which is exactly what the `rerun` reference otherwise saves you.
...
So treat the 1 MiB cap as the in-pod limit it is, rather than something `fetch` routes around.
