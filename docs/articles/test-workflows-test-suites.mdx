...
With the introduction of [Multi-Agent Environments](/articles/install/multi-agent) it is possible to specify target Runner Agents for each
...
Workflow is running on. For more details, see our guide on [Runner Agent Targeting](/articles/install/multi-agent#targeting-runner-agents-in-testkube-resources).
