...
In case you don't know an exact list of workflows to be executed inside of the suite, you can define it dynamically using test workflow labels.
...
An entry can be scheduled as a **rerun** of an earlier execution rather than as a fresh one, by naming that execution's ID:
...
The scheduled execution then resolves `execution("rerun")` back to that one, which is what lets a Workflow read the previous run's report and repeat only the test cases that failed. See [Rerunning Failed Tests](/articles/test-workflows-rerun#rerunning-from-inside-a-workflow) for the full pattern, the difference from a rerun started on the CLI, and the rule on which executions may be named.
...
:::tip
`config` passes data **into** an execution. To read data back **out** of one - the values it published, or the
artifacts it produced - see [Sharing Data Between Executions](/articles/test-workflows-execution-sharing).
:::
