...
To use this GitHub Action for [Testkube](https://app.testkube.io/), you need to create an [API token](https://docs.testkube.io/articles/organization-management#api-tokens).
...
- The **_GKE Service Account_** should be created prior in Google Cloud and added to GH Secrets along with **_GKE Project_** value;
...
:::note
GitHub Apps Integration is currently not supported for Testkube Cloud.
:::
...
### Additional configuration for the GitHub integration

The [GitHub integration](/articles/git-integration) (repository connections that run Test Workflows on PR events and post check-runs back) reuses the same GitHub App configured above, but needs extra App permissions, event subscriptions, and Helm values on top of what content cloning requires.

**App permissions** (add on top of `Contents: read`, `Metadata: read`):

- **Pull requests**: read & write, to post the aggregate PR comment.
- **Checks**: read & write, to write the aggregate check-run that gates merge.
- **Issues**: read, so the `@testkube` re-run comment trigger receives the `issue_comment` event.

**Subscribe to events** on the App:

- `Pull request`, `Push`, `Release`, `Issue comment`.

**Webhook URL**: point the App webhook at `https://<control-plane-host>/api/v1/github/webhooks` (the path served by `testkube-cloud-api`). Generate a webhook secret and keep it for the values below.

**Additional Helm values** on `testkube-cloud-api` (the feature is off by default and every endpoint returns "not configured" until `features.gitIntegration` is on, so the flag has to land alongside the App credentials):

```yaml
testkube-cloud-api:
  api:
    features:
      # turns on the GitHub integration endpoints, pipeline consumers, and the dashboard tab.
      # required — without it the App credentials below are read but the feature stays disabled.
      gitIntegration: true
    github:
      appId: "your_application_id"
      appSlug: "your-app-slug" # from the GitHub App URL, used to build install links from the dashboard
      installationId: "your_installation_id"
      privateKeyRef: "testkube-github"
      # webhook signature validation for PR/push/release deliveries
      webhookSecret: "your_webhook_secret"
      # or, if you keep the secret in a Kubernetes Secret:
      webhookSecretRef: "testkube-github-webhook"
      webhookSecretKey: "webhook-secret"
```

**Enable the GitHub integration worker** (a separate deployment that runs the repo-scan and webhook pipelines):

```yaml
testkube-git-provider-onboarding-worker:
  gitIntegration: true
  github:
    appId: "your_application_id"
...
Once the worker pod is running, users with `admin` on a repository can connect it from **Integrations > Git Integration > Connect repository** in the dashboard.

### Multiple GitHub Organizations

If you need to authenticate across multiple GitHub organizations, you can install the same application in each organization and then update `values.yaml` with a comma-separated list of IDs (for example, `installationId: "your_installation_id1,your_installation_id2"`).
...
The permissions above are not interchangeable: cloning needs `Contents: read` and
`Metadata: read`, and the extra grants exist only for the GitHub integration.

**`fatal: could not read Username for 'https://github.com': terminal prompts disabled`**

Git printing this during a clone step means it was handed no credentials at all, so the
App token was never issued. The usual cause is a mismatch between the permissions the
Control Plane asks GitHub for and the ones the installation was actually granted: GitHub
refuses to issue the token at all when the request is not a subset of the grant, and
answers `The permissions requested are not granted to this installation.` Since the clone
then runs unauthenticated, the message you see is the one from Git, not the one from
GitHub.

Check the installation's own permissions rather than the App's, as they can differ:

```
https://github.com/organizations/<org>/settings/installations/<installation_id>
```

**Changing App permissions does not update existing installations.** Adding a permission
puts the installation into a pending state until an organization owner accepts it, and
until then it keeps the permission set it had. Removing one applies without acceptance.
This is worth knowing when a permission change appears to have no effect.
