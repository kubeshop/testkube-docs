...
For each schedule, a special cron job is created from this [template](https://github.com/kubeshop/helm-charts/tree/main/charts/testkube/cronjob-template.yml).
...
After switching to the Testkube Agent's built-in cron scheduler, existing Kubernetes CronJobs will be automatically removed by the Agent during startup.
You can configure the Agent cron scheduler using the following helm chart variable:
