...
- **Resource Groups**: For the ultimate in fine-grained access control, Resource Groups allow you to restrict access to specific Testkube resources _across Environments_. This means you can have a general permission set within an Environment, but then override it for certain critical Test Workflows. It also allows you to enforce shared permissions across Test Workflows in multiple Environments (for example all Load Tests).
