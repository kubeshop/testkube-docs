...
organized into general best practices that apply to all workflows, recommendations for
specific test categories (performance, E2E/UI, API, etc.), error handling and scheduling patterns,
security guidelines, and tool-specific recommendations at the end.
...
Every workflow should declare [resource requests](/articles/test-workflows-job-and-pod) to help the Kubernetes scheduler place pods correctly
and avoid noisy-neighbor problems. You can use the [Resource Usage](/articles/testkube-dashboard-execution-details#resource-usage) tab in 
the Testkube Dashboard to monitor resource usage and help you adjust requests and limits accordingly.
...
See [Content](/articles/test-workflows-content#git-repository) for all Git options.
...
Give every step a descriptive `name`. Names appear in the Testkube Dashboard [execution logs](/articles/testkube-dashboard-execution-details) and make
...
Even if a test tool writes output to stdout, collecting file [artifacts](/articles/test-workflows-artifacts) (reports, logs, screenshots,
...
Protect your cluster from runaway tests by always setting [`activeDeadlineSeconds`](/articles/test-workflows-job-and-pod). Choose a value
...
set a [concurrency limit](/articles/test-workflows-concurrency-queueing):
...
Add [execution tags](/articles/execution-tags) to link test runs back
...
**Scale with [parallel](/articles/test-workflows-parallel) workers.** All major performance tools support distributed execution in
Testkube — see examples for [k6](/articles/examples/k6-distributed), [JMeter](/articles/examples/jmeter-distributed),
and [Locust](/articles/examples/locust-distributed). Use `parallel` to fan out load generation across multiple pods, and `paused: true`
...
Always write them to [artifacts](/articles/test-workflows-artifacts) so the team can review results without re-running the test.

**Use separate environments for performance tests.** Performance tests can disrupt shared services.
Use [execution tags](/articles/execution-tags) or
...
See the full examples for [Playwright](/articles/examples/playwright-basic), [Cypress](/articles/examples/cypress-basic),
and [Selenium](/articles/examples/selenium-basic).
...
`emptyDir` at `/dev/shm` to avoid crashes (see [Job & Pod Configuration](/articles/test-workflows-job-and-pod) for volume details):
...
[`parallel` and `shards`](/articles/test-workflows-matrix-and-sharding) features to split test files across workers and merge reports afterward.
See the [Playwright sharded](/articles/examples/playwright-sharded) and [Cypress sharded](/articles/examples/cypress-sharded) examples.
...
**Use [matrix execution](/articles/test-workflows-matrix-and-sharding) for cross-browser testing.** Run the same test suite across multiple browsers
...
See the full examples for [Postman](/articles/examples/postman-basic), [cURL](/articles/examples/curl-basic),
and [SoapUI](/articles/examples/soapui-basic).
...
**Use [matrix execution](/articles/test-workflows-matrix-and-sharding) for endpoint coverage.** Test multiple endpoints or environments in parallel
...
See the full examples for [Maven](/articles/examples/maven-basic), [Gradle](/articles/examples/gradle-basic),
[Pytest](/articles/examples/pytest-basic), and [NUnit](/articles/examples/nunit-basic).

**Collect build tool reports as [artifacts](/articles/test-workflows-artifacts).** Maven Surefire, Gradle, and pytest all produce JUnit
...
**Install dependencies before testing.** For Python-based tests, add an explicit setup step and mark it as [pure](/articles/test-workflows-high-level-architecture#pure-steps):
...
This is essential for artifact collection and cleanup if those are done in separate steps.
...
`until` [expression](/articles/test-workflows-expressions):
...
[Schedule](/articles/scheduling-tests) workflows to run on a cadence using the `events` property:
...
Compose larger test suites by having one workflow [execute](/articles/test-workflows-test-suites) others. Control parallelism to avoid
...
[test metrics](/articles/test-insights) or trigger [webhooks](/articles/webhooks) can be marked as silent:
...
Never hardcode sensitive values in your workflow definitions. Instead, use Testkube
[Credentials](/articles/credential-management) to store secrets (passwords, API keys, tokens) and
configuration variables (URLs, settings) securely. Credentials can be scoped at the organization,
environment, or workflow level — with workflow-scoped values taking the highest priority.
...
Use **encrypted** credentials for sensitive data (values are hidden in the UI and only injected at
runtime) and **plaintext** credentials for non-sensitive configuration that team members may need
to review or edit.

:::tip
You can manage credentials via the Dashboard under Organization Management, Environment Settings,
or Workflow Settings — see [Credential Management](/articles/credential-management) for details.
:::

### Use `imagePullSecrets` for Private Registries

If your test images live in a private registry, configure pull secrets at the [pod level](/articles/test-workflows-job-and-pod):
...
See the [basic](/articles/examples/k6-basic) and [distributed](/articles/examples/k6-distributed) k6 examples.
...
- Use the `official/k6/v1` [template](/articles/test-workflow-templates) for simple runs.

### Playwright

See the [basic](/articles/examples/playwright-basic), [sharded](/articles/examples/playwright-sharded),
and [rerun failed tests](/articles/examples/playwright-rerun) Playwright examples.
...
- Export [JUnit reports](/articles/test-workflows-reports) using `PLAYWRIGHT_JUNIT_OUTPUT_NAME`.
- Enable **traces** with `--trace on` for post-mortem debugging.
- Use **blob reporters** (`--reporter=blob`) for [sharded](/articles/test-workflows-matrix-and-sharding) runs, then merge with `playwright merge-reports`.

### Cypress

See the [basic](/articles/examples/cypress-basic) and [sharded](/articles/examples/cypress-sharded) Cypress examples.
...
- Use `--reporter junit` with `--reporter-options mochaFile=...` for [JUnit output](/articles/test-workflows-reports).
- Use the `official/cypress/v1` [template](/articles/test-workflow-templates) with the [`shards.testFiles`](/articles/test-workflows-matrix-and-sharding) option for parallelism.

### JMeter

See the [basic](/articles/examples/jmeter-basic) and [distributed](/articles/examples/jmeter-distributed) JMeter examples.

- Use the `alpine/jmeter` image for smaller footprint.
- Always generate all three output types: **log** (`-j`), **HTML report** (`-o`/`-e`), and **JTL** (`-l`).
- For **distributed testing**, use Testkube [services](/articles/test-workflows-services) for JMeter slaves:
...
- Connect the master to slaves using [expressions](/articles/test-workflows-expressions): `-R '{{ services.slave.*.ip }}'`.
- Use the `official/jmeter/v2` [template](/articles/test-workflow-templates) for simpler setups.

### Postman / Newman

See the [Postman example](/articles/examples/postman-basic).

- Use the `postman/newman:6-alpine` image.
- Always add [JUnit reporting](/articles/test-workflows-reports): `-r cli,junit --reporter-junit-export /data/artifacts/junit-report.xml`.
- Pass environment variables with `--env-var`.
- Use the `official/postman/v1` [template](/articles/test-workflow-templates).

### Artillery

See the [Artillery example](/articles/examples/artillery-basic).
...
See the [basic](/articles/examples/selenium-basic) and [advanced](/articles/examples/selenium-advanced) Selenium examples.

- Use **readiness probes** on the Selenium standalone or hub [service](/articles/test-workflows-services) to avoid tests starting before
...
- Pass the WebDriver URL via environment variable using [expressions](/articles/test-workflows-expressions):
...
See the [basic](/articles/examples/locust-basic) and [distributed](/articles/examples/locust-distributed) Locust examples.

- Use the **master-worker** architecture with [services](/articles/test-workflows-services) for distributed tests:
...
See the [Pytest example](/articles/examples/pytest-basic).

- Install dependencies in a dedicated step before running tests.
- Generate [JUnit XML](/articles/test-workflows-reports) with `--junit-xml=/data/artifacts/report.xml`.
- Use `python:3.x-alpine` images for smaller footprint when native dependencies are not needed.

### Maven / Gradle

See the [Maven](/articles/examples/maven-basic) and [Gradle](/articles/examples/gradle-basic) examples.
...
- For Maven, consider the `official/maven/v1` [template](/articles/test-workflow-templates).
- For Gradle, consider the `official/gradle/v1` [template](/articles/test-workflow-templates).

### .NET (NUnit, xUnit)

See the [NUnit example](/articles/examples/nunit-basic).

- Use the `bitnamilegacy/dotnet-sdk:8-debian-12` image (or equivalent).
- Generate [JUnit output](/articles/test-workflows-reports) with the logger option:
...
See the [basic](/articles/examples/robot-framework-basic) and [parallel](/articles/examples/robot-framework-parallel) Robot Framework examples.
...
- [Tool-Specific Examples](/articles/examples/overview)
