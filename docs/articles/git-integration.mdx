...
## Configuring event triggers with `.testkube/git-integration.yml`

Drop this file on the repo default branch to declare which Workflows run on which events:
...
- `workflows[].name` — Test Workflow name in the environment.
- `workflows[].required` — a failing run blocks the aggregate check.
- `workflows[].on.<event>` — per-event filter. Empty block matches any event of that kind.

The file is ignored for PRs originating from **forks** to avoid running untrusted YAML from external contributors.
...
Every execution scheduled by Git Integration carries `actor.type = gitintegration`. The **Executions** page has a **Git Integration** filter chip that returns those runs. The synthesized per-PR parent (`ql-parent-*`) is hidden from the list by design — the filter returns the child workflow executions that ran against the PR.
