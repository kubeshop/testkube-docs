...
**Advanced Git Integration** connects a GitHub repository to a Testkube environment so PR activity runs your Test Workflows and results post back on the PR as check-runs.
...
For every event received from GitHub, Advanced Git Integration:
...
Every auto-gen Workflow is labelled so Advanced Git Integration can reconcile it:
...
## Filtering executions by Advanced Git Integration

Every execution scheduled by Advanced Git Integration carries `actor.type = gitintegration`. The **Executions** page has a **Git Integration** filter chip that returns those runs.

## Re-scanning a repository

Framework detection is refreshed automatically when a PR event touches a stack anchor: `go.mod`, `package.json`, `pom.xml`, `build.gradle`, `build.gradle.kts`, `Cargo.toml`, `requirements.txt`, `pyproject.toml`, `Pipfile`, `Gemfile`, or `composer.json`. Advanced Git Integration re-runs detection on the PR head and updates the set of auto-gen Workflows before executing.
...
Comment `@testkube` on the PR. GitHub emits an `issue_comment` event and Advanced Git Integration runs the selected Workflows against the PR's current head. Useful for retrying a flake, re-checking after a base-branch fix, or triggering a run on a PR that opened before the integration was added.
...
Yes. You can keep Actions for build, lint, and deploy, and move the test run itself onto a Test Workflow that Advanced Git Integration executes. History, filtering, and AI summaries then live on the Testkube side and the aggregate check-run replaces your Actions test job.
...
Same job as a Test Workflow attached to Advanced Git Integration:
