...
Framework detection is refreshed automatically when a PR event touches a stack anchor: `go.mod`, `package.json`, `pom.xml`, `build.gradle`, `build.gradle.kts`, `Cargo.toml`, `requirements.txt`, `pyproject.toml`, `Pipfile`, `Gemfile`, or `composer.json`. Git Integration re-runs detection on the PR head and updates the set of auto-gen Workflows before executing.
...
Comment `@testkube` on the PR. GitHub emits an `issue_comment` event and Git Integration runs the selected Workflows against the PR's current head. Useful for retrying a flake, re-checking after a base-branch fix, or triggering a run on a PR that opened before the integration was added.
...
Yes. You can keep Actions for build, lint, and deploy, and move the test run itself onto a Test Workflow that Git Integration executes. History, filtering, and AI summaries then live on the Testkube side and the aggregate check-run replaces your Actions test job.
...
Same job as a Test Workflow attached to Git Integration:
...
Attach the Workflow during onboarding (or from the integration configuration panel later). On the next PR event, Testkube clones the head commit, runs the steps, and reports one aggregate check-run back to GitHub.
