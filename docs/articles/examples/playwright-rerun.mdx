...
:::note
Testing tools disagree on how a failed test is named and filtered, so translating "what failed" into a filter stays the Workflow's job. What a Workflow no longer has to work out is _which_ execution to narrow down from: Testkube records the execution a rerun descends from and exposes it as the reserved `rerun` reference, so `read_artifact("rerun", "test-results/.last-run.json")` reaches the previous run's results with no API token, no CLI call and no execution ID passed in - see [Rerunning Failed Tests](/articles/test-workflows-rerun).

The Workflow below predates that reference and is kept for the case it still covers: picking up the failures of a **previous, unrelated execution** on a fresh run, rather than on a rerun of that execution. For narrowing an actual rerun, prefer the `rerun` reference.
:::
...
- The CLI needs an API Token to retrieve results; it is recommended to use a `Member` token with access to the
  containing Environment with Read permissions as described at [Member Tokens](/articles/api-token-management#member-tokens)
