...
root tests folder (where Playwright will be default be looking for it).
...
:::note
Please note that the `rerun-failed` option should only be used for the same or "compatible" revision/"version"/branch of your underlying Playwright tests, 
just using the last one may not be correct.
:::
