...
The below Workflow shows how to leverage this functionality in Testkube alongside "regular" Playwright test execution.

- Playwright needs the `test-results/.last-run.json` file from a previous execution to rerun failed tests. The Workflow retrieves this by automating
  the Testkube CLI `testkube download artifact` command to download the corresponding file from a previous execution.
