...
- Run distributed K6 tests (`steps[0].parallel`)
  ...
  - It exports a per-worker `summary.json` with `--summary-export` and retrieves the generated HTML and JSON artifacts

<DistributedK6 />

Each worker uploads a unique `summary-worker-*.json` file. Testkube ingests these k6 summary reports as
[performance metrics](/articles/performance-metrics-in-insights).
...
![Distributed K6 HTML Report](images/distributed-k6-artifacts.png)
