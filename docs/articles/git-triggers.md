...
- To watch more than one of them with a single trigger, list them under `events` instead of `event` (see [Events](/articles/test-triggers#events)).
  A `content` trigger may list only git events, and the filters that apply are the union of those the listed events use —
  a trigger on both `git-push` and `git-pull-request` honours `branches` and `pullRequest.*`.
