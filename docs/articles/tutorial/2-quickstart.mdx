...
2. Request a trial license at https://testkube.io/get-started.
...
spec:
  ...
  # - without `workingDir` (or with empty) in step - it will use the parent's working dir (down the tree)
  # - without `workingDir` (or with empty) at all in the tree - it will use working dir from the image default
  # - with relative `workingDir` in step - it will go further from parent's working dir (down the tree)
  # - with relative `workingDir` in step, but no other in a tree - it will go further from the image default
  # - the parent is whatever is found in the tree, for example, the Global Template's `spec.container.workingDir`
