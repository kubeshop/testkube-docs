...
In this tutorial you will learn how to get started with Testkube, run your first Test Workflow, inspect its executions, and integrate it with your CI/CD pipeline(s). If you’d like to learn more about Testkube before deploying, [schedule a conversation with our Solutions Engineering team](https://testkube.io/demo).

**[Schedule time with our Solutions Engineering team](https://testkube.io/demo)** to get your trial license and answers to any questions you have.

:::note
Testkube is composed of two core components. The first is the Control Plane, which exposes the UI Dashboard, and facilitates management of Test Workflows, data, users, etc. The second component is the Test Execution Agent. One or more Agents can connect to a Control Plane, usually with one Agent per cluster, and do the work of running tests (scheduling jobs and managing pods). Learn more about our product architecture in the [Installation Overview](https://www.notion.so/articles/install/overview).

The Testkube Control Plane has flexibility in its deployment:

- **Testkube On-Prem** runs the Control Plane in your infrastructure. Install Testkube Runners separately when you want to execute tests.
- **Testkube Cloud Control Plane** runs the Control Plane in our cloud and the Test Execution Agents in your infrastructure.

With that in mind, we have a few recommendations on how to start with Testkube:

- If you are looking to evaluate Testkube before sharing it with others, we recommend using the [Evaluation Install](https://docs.testkube.io/articles/tutorial/quickstart/individual-evaluation). This runs on your local machine, and is deployed into a pre-existing light-weight Kubernetes distribution (like [minikube](https://minikube.sigs.k8s.io/docs/)). This guide assumes an Evaluation Installation.
- If you are looking to share Testkube with your team, then we recommend using the Cloud Control Plane, or [deploying an On-Prem Control Plane via Helm](https://docs.testkube.io/articles/install/install-with-helm) to a cluster which your team, or partner DevOps team, can manage. If you self host, we recommend deploying the Control Plane to your cluster and installing one or more Testkube Runners where your tests should execute, unless your use case requires otherwise.
  :::
...
2. Run the `testkube init demo` command in your terminal to install the Control Plane.
3. Install a Testkube Runner separately using the `testkube install runner` flow.
4. Run the `testkube dashboard` command to open Testkube without configuring Ingress.
5. Log in to Testkube with the credentials from the second step.

:::warning
This evaluation installation is for a **single/local user only**, it is not configured to be shared with other users, read more about evaluation
options in the [Quickstart Overview](/articles/tutorial/quickstart/overview).
...
<TabItem value="demo" label="Testkube Sandbox Environment (no install)">
...
:::info
The Sandbox Environment won't allow you to follow the steps further down in the quickstart since
those steps require your own environment.
:::

1. Sign in to the [Testkube Cloud Control Plane](https://app.testkube.io).
2. Once authenticated, [go to the sandbox environment.](https://app.testkube.io/demo-environment) - (there are multiple in-app
...
```bash
kubectl apply -f my-workflow.yaml && testkube run tw my-test --watch
```
...
testkube get artifacts 664dbae9924211bb076bd58e

# Download the artifact of this execution.
testkube download artifact 6655cc6d7d0abf889e531168 junit.xml .

# Download the logs of this execution.
testkube get twe 664dbae9924211bb076bd58e --logs-only
```
