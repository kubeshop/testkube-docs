...
| **Pro credentials** | API key, cp root domain, organization ID, and environment ID from your Testkube Pro account (for `pro connect` only). |
...
| Action | Command |
|--------|---------|
| Connect to Pro (full data migration) | `testkube pro connect --api-key <key> --org-id <org> --env-id <env> --root-domain <domain>` |
| Connect to Pro (skip data migration) | `testkube pro connect --api-key <key> --org-id <org> --env-id <env> --root-domain <domain> --skip-export` |
| Connect to Pro (recent data only) | `testkube pro connect --api-key <key> --org-id <org> --env-id <env> --root-domain <domain> --since 2025-01-01` |
