...
You can get the required tokens from the [Agents Panel](/articles/agents-overview#managing-agents).

:::tip
When creating a new Environment in the Testkube Dashboard, you will be shown the above command with the correct
parameters automatically - see [Creating Environments](/articles/environment-management#creating-a-new-environment).
:::
...
Once running, the Agent will connect to the Testkube Control Plane with all capabilities enabled (Runner, Listener, GitOps, Webhook) and be ready for running Workflows, listening for Kubernetes events, and more - [Read More](/articles/agents-overview).
...
[Agent Settings](/articles/agents-overview#managing-an-existing-agent) popup.
...
:::info
As of Testkube v2.7, Testkube Resources are stored in the Control Plane, so replacing the Docker Agent
will not result in loss of your Workflows or other resources - [Read More](/articles/testkube-resource-management).
For pre-v2.7 environments, this option comes with the drawback that you will lose any Testkube resources 
that were created in the Environment (since these were stored as CRDs in the Agent's cluster).
:::
...
:::info
Although the Testkube Docker Agent is a great way to run Testkube tests in your infrastructure, it is **not** meant
