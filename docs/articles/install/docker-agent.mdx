...
1. By upgrading the Agent inside an existing Docker Container
2. By replacing the entire Testkube Agent Docker Image with a new version

### Upgrading the Agent inside the Container

Use `docker exec` to run the Helm upgrade command provided in the
[Environment Settings](/testkube-pro/articles/environment-management#agent-information) tab, for example

```shell
➜  ~ docker exec <container id> helm repo add kubeshop https://kubeshop.github.io/helm-charts ; helm repo update && helm upgrade --install --create-namespace testkube kubeshop/testkube --set testkube-api.cloud.key=tkcagnt_XXX -set testkube-api.cloud.orgId=tkcorg_YYY --set testkube-api.cloud.envId=tkcenv_ZZZ --set testkube-api.minio.enabled=false --set mongodb.enabled=false --set testkube-dashboard.enabled=false --set testkube-api.cloud.url=agent.testkube.io:443 --namespace testkube
"kubeshop" has been added to your repositories
...
Release "testkube" has been upgraded. Happy Helming!
NAME: testkube
LAST DEPLOYED: Tue Oct  8 12:53:14 2024
NAMESPACE: testkube
STATUS: deployed
REVISION: 2
TEST SUITE: None
...
This is the easiest upgrade but comes with the drawback that you will lose any Testkube resources (Workflows, etc.)
