...
The Testkube Docker Agent provides a convenient way to evaluate Testkube Cloud as it allows you to run the Testkube Agent directly under
Docker without having to install or manage your own Kubernetes distribution.

:::info
You can, of course, use the Docker Agent with On Premise installations also, but it is not recommended for production usage - [Read More](#not-for-production-usage)
:::
