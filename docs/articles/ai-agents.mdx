...
creating a new agent. Templates provide pre-configured prompts and tool selections for common use cases.

Read more at [Creating an AI Agent from a Template](/articles/defining-ai-agents#creating-an-ai-agent-from-a-template).
...
[add custom LLM providers](/articles/ai-models) and select which model to use per conversation
