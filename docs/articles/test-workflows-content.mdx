...
**Explanation**: Under the hood, it will create EmptyDir volumes and volume mounts at /data/repo and /operator and clone the repositories there.
...
and you can then just clone it as above, but without volume.
...
**Explanation**: it will clone the repos into the ephemeral storage where image is unpacked. 
...
**Explanation**: Testkube will load the repository, while the prepared submodules will be available too
