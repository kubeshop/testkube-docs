...
Prefer a personal access token instead? See [Finding Your Git Provider Token](/articles/git-provider-tokens)
for where to create one in GitHub, GitLab, or Bitbucket.
...
Control how much logging the Git clone produces with `content.git.verbosity`.
Allowed values are `quiet`, `normal`, and `verbose`. If omitted, the clone defaults to `verbose`.
...
Git clone retries transient failures in-process. Configure the policy with `content.git.retry`:

- `count` — maximum attempts (default `5`, minimum `1`, maximum `20`)
- `delay` — base delay between attempts (for example `"100ms"` or `"1s"`); exponential backoff is applied.
  May use a [Test Workflow Expression](./test-workflows-expressions.md); the value is validated after resolution.
  The base delay is capped at `30s`.
...
:::info
This is separate from [step-level `retry`](/articles/test-workflows-examples-basics#retry-mechanism), which retries an entire Workflow step.
`content.git.retry` only retries the Git operations inside the clone.
:::
...
The cloned repositories will be available **only for nested steps**.
...
steps:
  - content:
      git:
        ...
        mountPath: /operator
    # highlight-start
    # NOTE: Cloned data only available for these nested steps
    steps:
