# Trigger Testkube webhook with Argo Events
...
Configure an EventSource that will watch the resources like a ConfigMap for ADD, UPDATE, and DELETE events and filter them if they have required label set.
...
c. View Sensor logs to verify that Testkube webhook is triggered.
...
d. View Test Workflow execution details using [Testkube CLI](/articles/install/cli) or Testkube Dashboard. Use the following command.
