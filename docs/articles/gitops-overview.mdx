...
Git the natural source of truth for your test definitions, triggers, and execution policies.
...
- **Storing tests as Kubernetes CRDs** - Test Workflows, Triggers, and Templates live alongside your application 
...
The following diagram illustrates how Testkube integrates into a typical GitOps workflow:
...
flowchart TB
    ...
    subgraph cluster["Kubernetes Cluster"]
        ...
        subgraph testkube["Testkube Namespace"]
            agent["Testkube Agent"]
            tw["TestWorkflow CRDs"]
            tt["TestTrigger CRDs"]
        end
    end

    subgraph cp["Testkube Control Plane"]
        dashboard["Dashboard"]
        storage["Results & Artifacts"]
    end

    git --> |"sync"| gitops
    gitops --> |"reconcile"| deployment
    gitops --> |"reconcile"| services
    gitops --> |"reconcile"| tw
    gitops --> |"reconcile"| tt
    
    deployment --> |"events"| agent
    agent --> |"execute"| tw
    tt --> |"trigger"| agent
    agent --> |"report"| cp
```
...
When using GitOps with Testkube, test executions can be triggered through multiple mechanisms. The sequence 
diagram below shows the typical flow when a deployment change triggers test execution:
...
sequenceDiagram
    ...
    Git->>GitOps: Push changes
    GitOps->>K8s: Sync resources
    K8s->>K8s: Update Deployment
    K8s->>Agent: Deployment modified event
    Agent->>Agent: Match TestTrigger
    Agent->>K8s: Create Workflow Job
    K8s->>K8s: Execute test containers
    Agent->>CP: Stream logs & results
    CP->>CP: Store artifacts
```
