...
Test Orchestration Platform, Testkube stores all its core resources as Custom Resource Definitions (CRDs), making 
...
While ArgoCD and Flux are the core GitOps tools, Testkube also integrates with complementary 
delivery and automation tools:

- **[Progressive Delivery](/articles/argorollouts-integration)** - Use Testkube as an analysis provider 
  for Argo Rollouts or Keptn to validate canary/blue-green deployments before promotion.
  
- **[Event-Driven Automation](/articles/argoevents-integration)** - Trigger test executions based on 
  cluster events using Argo Events, orchestrate complex test pipelines with Argo Workflows, or 
  emit standardized CDEvents for CI/CD interoperability.
