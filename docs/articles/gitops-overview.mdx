...
platform, Testkube manages all its core resources as Custom Resource Definitions (CRDs), making
...
Although Testkube Resources are stored in the Testkube Control Plane, they can still be provided and managed as CRDs in an external/GitOps setup by using
...
- If you have multiple GitOps Agents syncing the same Testkube Resources into the same Testkube Environment, these will overwrite each other unless
  [GitOps Resource Ownership](#gitops-resource-ownership) is enabled.

### GitOps Resource Ownership

When several GitOps Agents sync into the same Environment, they can target the same resource name and silently overwrite each other.
**GitOps Resource Ownership** makes that exclusive: the first GitOps Agent to sync a resource claims it, and create, update, and delete
syncs from other GitOps Agents are rejected until you transfer ownership.

Enable it under **Organization Management → Product Features → GitOps Resource Ownership**. The feature is off by default.

Ownership applies to all resource types synced by the GitOps Agent: TestWorkflows, TestWorkflow Templates, TestTriggers,
WorkflowTriggers, Webhooks, and Webhook Templates.

#### Transferring Ownership

To assign or transfer ownership, set the `testkube.io/gitops-owner` annotation on the Kubernetes CRD to the target Agent ID
(not the agent name):
...
metadata:
  ...
  annotations:
    testkube.io/gitops-owner: "<agent-id>"
```

The Agent ID is shown in the Agents list under Environment Settings.
...
> A GitOps Agent is deployed into Namespace A which contains Workflow B. The Testkube Environment the Agent is connected to already contains another Workflow C.
>
...
### Temporarily Pausing GitOps Sync
...
This applies to all resource types synced by the GitOps Agent: TestWorkflows, TestWorkflow Templates,
TestTriggers, Webhooks, and Webhook Templates.

#### Where to Place the Annotation

You can add the annotation in **either** location depending on your use case:

- **On the CRD in Kubernetes** -- prevents that specific CRD from ever being synced to the Control
  Plane. Useful when a resource exists in the cluster for other purposes but should not appear in the
  Testkube Environment.
- **On the resource in the Control Plane** (via the Dashboard or API) -- protects the Control Plane
  copy from being overwritten or deleted by any GitOps Agent. Useful when you need to make temporary
  manual edits in the Control Plane without having them reverted on the next reconciliation cycle.

:::info
Because the annotation is checked on both the incoming CRD and the existing Control Plane resource,
adding it in **either** location is sufficient to block sync for that resource.
:::

#### Re-enabling Sync

To re-enable sync, remove the annotation (or set it to `"false"`) and then apply your desired
resource state again from Git. If the annotation was set on the Control Plane resource, you also
need to remove or update it there (via the Dashboard or API) before syncs will be applied again.
