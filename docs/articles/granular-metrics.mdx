...
| --- | --- | --- |
| k6 summary | Scenario, k6 metric tags | k6 only writes tagged submetrics to the summary when the script requests them, usually through thresholds. Global metrics such as total received bytes or max VUs may not have a scenario identity. |
| Artillery report | Usually workflow-level aggregate metrics | Standard Artillery JSON reports expose aggregate counters, rates, summaries, and endpoint-shaped plugin metrics. A named Artillery scenario is useful test metadata, but the exported aggregate report does not guarantee scenario- or transaction-scoped latency/request metrics. |
...
k6 does not include every possible tag combination in `summary.json` automatically. If you need scenario-level
series in Insights, define named scenarios and add thresholds for the metric/tag combinations you want to chart:
...
Artillery scenario definitions can have names, and those names are useful for keeping the script readable and for
running a specific scenario with Artillery. However, the standard JSON report exported with `-o` is primarily an
aggregate report. In current Testkube ingestion, Artillery metrics should be treated as workflow-level report
metrics unless the uploaded report contains an explicit stable identity that Testkube can store.

For Insights boards, this means Artillery latency, request rate, request volume, and error charts should usually be
segmented by workflow or left unsegmented. Do not expect **Scenario** or **Transaction** filter values for Artillery
metrics unless your report format actually includes those dimensions.
