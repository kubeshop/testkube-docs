...
To capture **custom metrics** from your own scripts or tools, write them in [InfluxDB Line Protocol](#custom-metrics-with-influx-line-protocol)
format and upload the file as a workflow artifact. Testkube also ingests metrics automatically from JUnit reports and
from supported performance-testing tools such as k6, Artillery, and JMeter.
...
benchmarks, database query timings, or ad-hoc shell measurements — without waiting for a dedicated report integration.
...
Insights under **Custom Report Metrics** (unless the metric key matches another Insights group, such as a latency key
under **Request Latency**).
...
```text
measurement[,tag_key=tag_value...] field_key=value[,field_key=value...] [timestamp]
```
...
- **Timestamp** — optional nanosecond Unix timestamp at the end of the line. When omitted, Testkube associates the
  values with the workflow execution's finish time.
...
cpu,host=server01 usage_user=0.10,usage_system=0.20 1670000000000000000
```
...
Tags on each line become identity fields, similar to k6 metric tags. For the record
`api_latency,endpoint=checkout,status=200 p95=245.5`, you can segment or filter by **endpoint** or **status** in
Insights when charting `api_latency_p95`.

Use stable, low-cardinality tag keys — such as `endpoint`, `scenario`, or `service` — rather than unique IDs or raw
