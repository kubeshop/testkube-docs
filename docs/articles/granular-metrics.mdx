...
and iteration duration as individual metric series. Testkube stores those series with the workflow execution,
then makes them available for Insights analysis across multiple executions.

:::note
Granular metrics are execution-level metric series. A k6 `summary.json` file contains aggregated values for one
run, such as `avg`, `min`, `max`, `p(90)`, `p(95)`, `count`, and `rate`. Testkube stores those values per
execution so you can compare them over time. It does not turn a k6 summary export into raw per-sample metrics
from inside a single k6 run.
:::
