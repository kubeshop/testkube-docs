...
The error fields tell a suite why a child execution failed. The [reason codes](/articles/test-workflows-failure-reasons) are stable, and the messages are words for people, so an assert reads the code where one exists:
