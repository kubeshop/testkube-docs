...
| `errorMessage` | Message of the initialization step, empty when it has none                                  |
| `errorReason`  | Reason code of the initialization step, empty when it has none                              |
| `stepErrors`   | Map of the step messages, keyed by step reference                                           |
| `stepReasons`  | Map of the step reason codes, keyed by step reference                                       |
| `stepAttempts` | Map of the number of attempts per step, for the steps that ran more than once               |
| `statusType`   | The type of the status details, for example `init-failure`. Empty when the execution passed |
| `statusReason` | The reason code of the status details, for example `unschedulable`                          |
| `statusStep`   | The reference of the step that holds the cause, empty for the initialization step           |

The error fields tell a suite why a child execution failed. The [reason codes](/articles/test-workflows-failure-reasons) do not change between versions, while the message is text for people. So an assert compares the code when the failure has one. The [status details](/articles/test-workflows-failure-reasons#the-status-details-of-an-execution) give the same answer for every failure, in three fields:

```yaml
- name: Check why the child failed
  shell: |
    [ '{{ execution("p").errorReason }}' = 'unschedulable' ] || { echo 'unexpected reason'; exit 1; }
```
