...
The error fields tell a suite why a child execution failed. The [reason codes](/articles/test-workflows-failure-reasons) do not change between versions, while the message is text for people. So an assert compares the code when the failure has one:
...
The first argument identifies the execution. Five forms work:
...
| `"rerun"`     | The execution the current one is a rerun of — see [Rerunning Failed Tests](/articles/test-workflows-rerun) |
...
`parent` and `rerun` are reserved. If the same Workflow also runs something under one of those names, the reference is refused rather than silently resolving to one of the two, and the error says to rename the alias.
