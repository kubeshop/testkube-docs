...
An execution publishes values the same way a step does: write to `/testkube/outputs/<key>`. The value is readable by later steps of the same Workflow **and** by the Workflow that ran it.
...
Nothing in the producer declares that the value leaves the Workflow — every output is readable by whoever ran it.
...
Outputs are only complete once an execution finishes. An `async` execution has not published anything yet by the time the step continues.
