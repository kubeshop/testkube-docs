...
Please take into account that timeout includes all time periods starting from creation of the kubernetes pod, scheduling it on the node, pulling the image, waiting for its readiness and executing the test request against the service.
