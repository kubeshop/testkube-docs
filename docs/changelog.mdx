...
Control Plane is available in Helm chart `[TODO]` and the Agent is available in Helm chart `[TODO]`.

### What's new!

#### Granular Test Insights

Monitoring test results across multiple tools and workflows can get noisy fast. In v2.10.0, we expanded Insights so you can do a deep dive into your performance and correctness tests.

- Automatically process JUnit, JMeter, k6, and Artillery reports, extracting relevant metrics.
...
Teams on a single plan can now manage multiple organizations from one place: create organizations, review usage across them, and get warnings before hitting plan limits. GitOps-managed workflows also warn you when dashboard edits may be overwritten by sync.

#### Improvements

- Multi-organization support on a single plan: create organizations, see usage across your organizations, and get warnings when approaching plan limits.
- Added a warning when editing GitOps managed workflows, so it’s clearer when UI changes may be reverted.
