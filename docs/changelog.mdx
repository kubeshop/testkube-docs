...
**Executions dashboard optimizations**
- The executions dashboard was redesigned for a lighter, more streamlined view — making it easier for users to focus on executions.
- A dedicated “Executions” entry was added to the main navigation, with built-in filters plus options for creating custom, advanced filtering.
- Users can now build custom views with specific filters, and reuse those views later.
- The detail page for a single execution is now more accessible and loads significantly faster.

**Webhooks support for global credentials&variables**
Webhooks now support global credentials and variables, enabling a more secure and streamlined configuration process.
...
- Tracking dashboard events for Sandbox account
- Updating credential now updates the correct credential scope when there is an override
- Posthog is now routed through a proxy to avoid ads blockers issues
