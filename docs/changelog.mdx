...
This release brings smarter filtering, personalized views, enhanced observability, and streamlined enterprise integrations.

**Dynamic filters** now automatically show only the options that are actually available in your current data, **eliminating UI clutter and making filtering more intuitive.** No more scrolling through irrelevant filter options.
...
**Enhanced Prometheus metrics** now include **additional workflow execution fields (triggered_by, tags) plus new step-level duration and count metrics** for deeper visibility into Test Workflow performance and execution details.
...
**New Testkube MCP Server** enables **AI assistants (GitHub Copilot, Cursor, Claude Desktop) to directly run and monitor Test Workflows**, analyze results, search execution history, and create workflows. AI agents can now help automate debugging and identify failing patterns right from your coding environment.

### Improvements and fixes

- Added dynamic filtering system that shows only available options based on current data to reduce interface clutter
- Added private filters - save personalized filter views that only you can see, with easy management through a new navbar section
...
- New and enhanced Agent Prometheus metrics
- Added support for SCIM in SaaS and improved user experience for on-prem as it is now configurable via UI.
