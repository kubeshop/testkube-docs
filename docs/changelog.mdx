...
- Enhanced the Insights Board user experience with support for pinned boards, shared/privated boards, and search results by name
...
- Added a default Insights Board at the Workflow level with a default set of charts
...
- Metrics
  - Canonicalized metric names: k6, Artillery, and JMeter now share normalized `latency_p95_ms`, `throughput_rps`, and `errors_rate` alongside the
     tool-specific names, so you can compare runs across tools in one chart and filter by source
