...
and now they are securely referencing **Credentials**, so you can inject API keys and secrets into webhook URLs, headers, or payload templates using the ``credentials()`` expression instead of hard-coding them.
