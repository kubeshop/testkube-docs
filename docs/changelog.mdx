...
## Monthly Release v1.21.0 (2025-12-10)

This release is available in Helm chart v2.327.0.
...
Last year (2024), with the introduction of TestWorkflows, we announced our plan to deprecate legacy features including Tests, Suites, Executors, and Sources [see documentation on legacy features](https://docs.testkube.io/articles/legacy-features).
:::danger
...
To simplify our codebase and improve development velocity, we are now moving forward with the removal of these legacy components from the Control Plane and Dashboard.
Since the majority of users have already migrated to TestWorkflows, we expect minimal impact. For teams that still rely on these features, a backward-compatible legacy version will remain available for a limited transition period, receiving only critical fixes (no new functionality).
