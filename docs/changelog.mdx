...
Workflow CRDs are no longer synced from Kubernetes by default in connected mode.
...
**Webhook and trigger execution remain agent-based**

- Webhooks continue to be executed by Agents via the agent capability model.
- Test Triggers continue to run through [Listener Agents](/articles/agents-overview#listener-agents) (or Standalone Agent listener behavior).

Note: "webhook capability" is currently an internal implementation detail and may change in future releases.

Read more: [Control Plane Source of Truth](/articles/control-plane-source-of-truth#webhooks-and-triggers)

### Observability updates

**Control Plane metrics enabled by default**

Control Plane metrics are enabled by default in `v2.7`.
Scheduler metrics docs are consolidated with control-plane metrics documentation.
