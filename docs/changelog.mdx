...
## Patch Release v2.13.3 (2026-09-22)

Agent only. The Agent is available in Helm chart `v2.13.3`; the Control Plane stays on `v2.336.2`.

### Improvements

- On-prem trial telemetry: license installation and validation activity is now tracked, and CLI `init demo` telemetry uses the license identity.

## Patch Release v2.13.2 (2026-09-22)

Control Plane only. The Control Plane is available in Helm chart `v2.336.2`.

### Fixes

- Workflow template Definition view no longer hangs. Switching between template tabs now keeps the loaded template instead of resetting it, so the Definition and Resource Group views open normally again.
- Template updates now answer with the updated template on source-of-truth environments.

## Release v2.13.1 (2026-08-27)

> Advanced GitHub-native integration, test case-level Health metrics and Insights, plus many smaller improvements and fixes.

Control Plane is available in Helm chart `v2.336.1` and the Agent is available in Helm chart `v2.13.1`.
...
- Read more about using the advance GitHub Integration [here](/articles/git-integration).
...
- Prometheus metrics:
  - TestWorkflow execution metrics now include `workflow_namespace`.
  - Webhook CRUD operations now emit Prometheus counters for create, update, and delete events.
  - GitOps/sync-based TestWorkflow and TestTrigger CRUD operations now appear in Prometheus metrics, including error outcomes.
  - A new `testkube_testtrigger_events_total` metric tracks workflow executions started by a `TestTrigger`, labeled by trigger name.
  - Optional metadata metrics can now export custom TestWorkflow labels through `testkube_testworkflow_custom_label_info`.
    For more details, see the [Control Plane Metrics documentation](/articles/control-plane-metrics).
...
> Private AI chats, deeper Insights and Analytics, plus major improvements across security, observability, CLI workflows, and runner operations.
...
- Testkube can now observe a Git repository and trigger executions on repository events. This covers the full lifecycle from simple branch pushes to GitHub pull-request automation, with fine-grained filtering and rich Git context exposed to the triggered execution ([read more](articles/git-triggers))
- Test Triggers can now watch custom resources and fire on specific field changes, not just built-in Kubernetes events ([read more](articles/test-triggers#triggering-on-custom-resources-crds)). Point a trigger at any CRD, such as Argo Rollouts, cert-manager, or your own, and add match conditions so it only runs when a field you care about changes, for example when a Rollout's `.status.phase` becomes `Healthy`. The dashboard discovers the resources each listener agent can watch, autocompletes their fields from the live schema, and lets you pin the trigger to the listener that should evaluate it
...
- Trial users of our Testkube Cloud control plane now get an automatically provisioned, cloud hosted runner set up for them so they can start running
  test workflows straight away. Users can still add their own agents as well, of course.

### Insights Updates

- Enhanced the Insights Board user experience with support for pinned boards, shared/privated boards, and search results by name - ([read more](/articles/test-insights#managing-custom-boards))
...
- Added a default Insights Board at the Workflow level with a default set of charts - ([read more](/articles/test-insights#the-default-workflow-insights-board) )
- Added eight chart types in Insights for better analysis of execution metrics ([read more](/articles/test-insights#chart-types))
- Granular Metrics
  - Native parsing of k6, Artillery, and JMeter metrics into Insights - ([read more](/articles/performance-metrics-in-insights))
  - Canonicalized metric names: k6, Artillery, and JMeter now share normalized `latency_p95_ms`, `throughput_rps`, and `errors_rate` alongside the
    tool-specific names, so you can compare runs across tools in one chart and filter by source - ([read more](/articles/performance-metrics-in-insights#canonical-cross-tool-metrics))
  - Added support for Influx Line Protocol for creation of custom metric series - [read more](articles/custom-metrics-in-insights)

### Operations & Security

- Testkube can now authenticate to GitHub through a single centralized Testkube GitHub App. One Testkube organization can connect to multiple GitHub organizations and access repositories across all of them ([read more](articles/github-app-auth))
...
- Added a new maxRole ceiling that lets you lock a member's effective permissions to read-only regardless of the roles they hold on individual environments or teams ([read more](articles/member-management#read-access-controls-org-wide-read-tokens--the-read-only-user-cap))
...
- Runner lease coordination configuration:  New environment variables on the agent to control its Kubernetes lease coordination:
  - `LEADER_ELECTION_DISABLED`  -> disables lease coordination for explicitly single-replica runner deployments . Default: `false`
  - `LEASE_CHECK_INTERVAL`  ->  how often the agent checks/renews the lease (e.g. `30s`, `1m`). Default: `5s`
...
- TestTrigger templates: support JSON-style field names (e.g. `.metadata.labels`) for built-in resources, matching `resourceRef`.
- Cron job target configuration: cron job configuration in TestWorkflow Template allow target property to schedule executions in specific agents.
...
In v2.10.0, we've expanded the underlying metrics data model for Test Insights so you can do a deep dive into your
E2E/functional and performance test results over time.
...
On-Prem Customers on the Enterprise plan can now manage multiple organizations from one place: create organizations,
review usage across them, and get warnings before hitting plan limits.
...
Read more: [Viewing Open Source Executions](/articles/viewing-oss-executions)
...
- **MongoDB to PostgreSQL Migration**: Added `skip_tables` parameter to the convert utility for Mongo to Postgres migration.
...
- **Missing `IMPORTS_BUCKET` variable**: Added the missing `IMPORTS_BUCKET` environment variable for both `cloud-api` and `worker` services. By default, it now falls back to `OUTPUTS_BUCKET` unless a different imports bucket is explicitly configured. This variable is used for importing OSS data into Testkube Cloud; because it was missing, upgrades of the charts to v2.9.1 were failing.

### Features

- **Disable test log persistence**: Added a `none` option for `logs.storage` to disable test log persistence entirely for environments that don't need to retain log data.
- **S3 Virtual-Hosted-Style URLs**: Added support for S3 virtual-hosted-style URLs (`bucket.s3.region.amazonaws.com`) alongside the existing path-style addressing.

### Fixes

- **Container security context on init containers**: Fixed an issue where `ContainerSecurityContext` could not be applied to init containers in TestWorkflow pods.
- **Floating runner parallel scheduling**: Fixed an issue where an already-active floating runner could be blocked from taking additional parallel executions when the floating runner concurrency limit was set to `1`.
- **Quiet workflow log streams**: Fixed an issue where workflow log streams could disconnect during quiet periods even though the workflow was still running.
- **TestWorkflowExecution controller status generation**: Fixed `Status.Generation` never being written by the `testworkflowexecution` controller, which caused all TWE CRDs to re-trigger on every operator restart.

### OSS Execution Viewer

- **Validate execution before prompting**: `testkube view` now validates the execution ID (or name) before asking for upload permission, so users see a clear "execution not found" error instead of being prompted for non-existent executions.
- **Log panel scroll**: Fixed an issue where the execution viewer expanded log panels to the full log height, breaking the internal scroll. Logs now scroll within their fixed container.
- **Avoid duplicate uploads**: `testkube view <id>` now checks whether the execution has already been shared and reuses the existing public link instead of re-uploading the same execution.
...
Open Source users can now view execution details (step tree, logs, and artifacts) from their TestWorkflow executions
directly in their browser, in a lightweight, read-only page hosted by Testkube. No login or Cloud account required.
...
TestTriggers can now fire on arbitrary Kubernetes Custom Resources, not just the built-in set (Pods, Deployments, etc.).
Any CRD installed in the cluster — Argo Rollouts, Flux, Crossplane, your own operator's types — can be named as the trigger's resource by its Group, Version, and Kind.
...
- **Smoke tests after each canary step** — match on `status.phase == Paused` to run quick health checks while the Rollout is waiting at a weight increment.
- **Full regression on completion** — match on `status.phase == Healthy` and `status.currentPodHash` changing, so tests run exactly once per successful rollout and not on unrelated status updates.
- **Rollback validation** — match on `status.phase == Degraded` to trigger a diagnostic workflow the moment Argo aborts a rollout.
...
local agent and uploads it to the Testkube Control Plane, ensuring seamless data migration. It then installs a [Testkube Runner Agent](/articles/agents-overview),
...
- **Automatic data migration**: Execution data is exported as a `.tar.gz` archive and imported to the Control Plane in a single flow — no manual steps required.
- **Graceful error handling**: If the export or import fails (e.g. archive too large), users are warned and can continue connecting; the archive is preserved for manual retry.
...
- **CRD conflict prevention**: The runner chart is installed with `gitops.installCRD: false` to avoid duplicate CRD ownership with the existing OSS chart.

#### `testkube pro disconnect`

Reverts a Testkube Pro installation back to OSS standalone mode. The command uninstalls the Testkube Runner Agent chart, deletes the agent record from the Control Plane,
scales the original OSS services (API server, MinIO, NATS, and the database) back up, and resets the CLI context to kubeconfig mode. All historical data remains
intact in the original cluster database.
...
- **Configurable replicas**: Users can control how many replicas to restore for MinIO (`--minio-replicas`), MongoDB (`--mongo-replicas`), and PostgreSQL (`--postgres-replicas`).
...
Added CLI commands to browse and install TestWorkflows from the Testkube Marketplace - [Read more](/articles/examples/marketplace#using-the-marketplace-from-the-cli)
...
- The JUnit Report tab now automatically opens the first failed test and also allows for filtering on skipped tests - [Read more](/articles/test-workflows-reports#2-reports-tab).
...
- Added support for running PostgreSQL with CloudNativePG while keeping the existing bundled PostgreSQL deployment for backward compatibility. This includes support for installing the CloudNativePG operator and provisioning an operator-managed PostgreSQL cluster. Read more: [PostgreSQL](/articles/install/standalone-agent#database)
...
- TestWorkflows are now easier to run on OpenShift and similar restricted clusters. You can opt out of automatic pod `fsGroup` injection when your platform expects security policies to assign that value.
...
- New! Testkube on-prem installations no longer require `testkube-agent`. New environments start with the Control Plane owning Testkube Resource management by default.
- `testkube init demo` no longer installs the agent bundle.
- SCIM Server lock contention: write lock contention now returns 409 instead of 429. This is the correct status per RFC 7644 Table 9 for POST/PUT/PATCH/DELETE conflicts. The library passes it through, and IdPs treat it as retriable.
...
- Added MongoDB jobs to upgrade from version `8.0.15` to `8.2.5`. [Read more](/articles/install/advanced-install#mongodb-upgrade-from-8015-to-825)
- SuperAgents can no longer be removed before migration.

### Agent v2.8.1

#### Improvements and fixes

- Added MongoDB jobs to upgrade from version `8.0.13` to `8.2.5`. [Read more](/articles/install/standalone-agent#mongodb-upgrade-from-8013-to-825)

## Monthly Release v2.8.0 (2026-03-26)

This release is available in Helm chart v2.331.0. Control Plane and Agent versions are now unified at **v2.8.0**. ([Check out Agent release notes](https://github.com/kubeshop/testkube/releases/tag/2.8.0)).
...
You can now use HashiCorp Vault (KV v2) as a backend in Credentials API. Workflow secrets are read from Vault at execution time instead of being stored in Testkube's database. Useful for teams that already manage secrets in Vault and do not want to duplicate them.
As of now, this is available only for on-prem installations.

Supports Kubernetes auth, static token auth, and Vault Enterprise namespaces.

Read more: [Credentials — Vault Backend Setup](/articles/credential-management#vault-backend-setup)
...
Read more: [Credentials](/articles/credential-management)
...
Read more: [Workflow Marketplace](/articles/examples/marketplace)
...
Read more: [Sharing Data Between Steps](/articles/test-workflows-step-sharing)
...
Read more: [Parallel Steps](/articles/test-workflows-parallel)
...
Read more: [Backstage Integration](/articles/backstage-integration)
...
Read more: [Testworkflow Timeouts](/articles/test-workflows-timeouts)
...
Read more: [GitHub Apps with Testkube](/articles/github-actions#using-github-apps-with-testkube)

#### Agent Key Rotation

The CLI now supports rotating agent secret keys. This is useful if you need to rotate the secret key for an agent.

Read more: [Agent Key Rotation](/articles/agents-overview#agent-key-rotation)

### AI Updates

#### Chat Overlay

A floating, resizable chat widget is now accessible from any page in the Testkube dashboard. Start a new AI conversation or switch between recent chats without leaving your current workflow. The widget persists its position and size across sessions.

Read more: [AI Agent Chats](/articles/using-ai-agents)

#### Chat Sidebar

A persistent side panel lets you work with AI assistance alongside your regular dashboard workflow. The sidebar supports multiple simultaneous chat tabs, each with independent conversation state. Panel width is resizable and remembered across sessions.

Read more: [AI Agent Chats](/articles/using-ai-agents)

#### AI Agent Triggers

You can now configure AI Agent triggers that automatically invoke an AI agent in response to test events. For example, set up a trigger that runs a failure-analysis agent whenever a Cypress test fails — with configurable trigger events, label selectors, trigger modes, and prompt templates.

Read more: [AI Agent Triggers](/articles/ai-triggers)

#### MCP Registry Browser

Browse and install MCP servers directly from the dashboard using the new MCP Registry Browser. Search available servers, view descriptions and versions, and install them with a single click to extend your AI agents' capabilities.

Read more: [Connected MCP Servers](/articles/mcp-servers-for-ai-agents)

#### Custom LLM Models

Environment administrators can now configure custom LLM providers and models for AI agents. Add your own OpenAI-compatible provider with an API key credential, register specific models, and select them in the chat model picker alongside the platform default.

Read more: [Configuring AI Models](/articles/ai-models)

#### Notifications for Pending Tool Approvals

A new notification center alerts you when AI agent sessions are waiting for tool approval. Review and approve pending actions directly from the notification panel without having to navigate to each chat session individually.

Read more: [AI Agent Chats - MCP Tool Approval](/articles/using-ai-agents#mcp-tool-approval)

#### Default Installed Agents

New Environments now come with 4 default agents tuned for Troubleshooting, Analysis, Optimization and General Assistance.

Read more: [Defining AI Agents](/articles/ai-agents#default-ai-agents)

### Improvements and fixes

- JUnit report attachments, including screenshots and URLs, are now rendered inline in the Report tab. [Read more](/articles/test-workflows-reports#2-reports-tab)
- Execution tags can now be edited after completion — through the Dashboard, CLI, or MCP server. [Read more](/articles/execution-tags)

## Patch Release v2.7.6 (2026-03-23)

This patch release focuses on runner registration and data parsing stability.

Control Plane is available in Helm chart v2.330.6.

### Fixes

- Fix a panic when using PostgreSQL when listing test workflows without executions.
- Remove the runner registration rate limiter to avoid unnecessary registration failures.

## Patch Release v2.7.5 (2026-03-20)

This patch release focuses on PostgreSQL database migration reliability.

Control Plane is available in Helm chart v2.330.5.

### Fixes

- Allow out-of-order PostgreSQL migrations so release upgrades are more resilient.

## Patch Release v2.7.4 (2026-03-20)

This patch release focuses on control-plane stability.

### Fixes

- Control Plane now emits resource lifecycle events consistently when resources are created, updated, or deleted through the Control Plane.
- Control Plane now uses MongoDB v2 client.
- `ListExecutions` MCP handler now handles empty `WorkflowName` and date filters correctly.
- Prevent a Postgres read-write race that could affect distributed locking behavior.
- Fixed Postgres empty TestWorkflows catalog.
- `queue-testworkflow` and `start-testworkflow` events now correctly trigger webhooks.

## Patch Release v2.7.3 (2026-03-16)

This patch release introduces fixes for Testkube CLI and Control Plane PostgreSQL BD support.

Control Plane is available in Helm chart v2.330.3.

### Fixes

- Include missing PostgreSQL DB table called `leases`.
- Returning a list when requesting server to run multiple Test Workflows. As instance, using Testkube CLI to start Test Workflows by label(s) when more than one match with same label(s).

## Patch Release v2.7.1 (2026-03-10)

This patch release improves webhook reliability, Agent capability visibility, and runner installation behavior.

Update both the Control Plane and the Agent to get the full set of fixes in this release. This release is available in Helm chart v2.330.1.

### Fixes

- `queue-testworkflow` and `start-testworkflow` events now correctly trigger webhooks.
- Agent capabilities are now synced to the Control Plane based on the Agent configuration, so the UI reflects how Agents are actually configured.
- `runner` capability is still not changed through capability sync. If you need to change it, recreate the Agent.

## Monthly Release v2.7.0 (2026-03-04)

This release is available in Helm chart v2.330.0. Control Plane and Agent continue with unified versioning in `2.7.x`.

### What's new!

#### Control Plane owns Testkube Resource management

Starting in `v2.7`, Testkube Resource ownership moves to the Control Plane for commercial deployments.
...
If you're upgrading from a Testkube previous version, your existing SuperAgent(s) will automatically migrate your Testkube Environment(s) with the new source-of-truth model.

Read more: [Testkube Resource Management](/articles/testkube-resource-management)

#### Workflow sync behavior in GitOps Deployments

If your team manages Testkube Resources as CRDs, enable and validate newly introduced **GitOps Agent** so Git-managed CRD updates continue to reconcile as expected.

Read more about [GitOps with Testkube](/articles/gitops-overview) and [Testkube Agents](/articles/agents-overview).
...
The legacy execution groups endpoint was removed in `v2.7.0`:
`GET /organizations/{orgId}/environments/{environmentId}/test-workflows/{workflowId}/execution-groups`

-> Use the executions list endpoint instead:
`GET /organizations/{orgId}/environments/{environmentId}/agent/test-workflows/{workflowId}/executions`

#### Kubernetes lifecycle events

If you rely on [Kubernetes Events](/articles/k8s-events#resource-lifecycle-events) or [CDEvents](/articles/cd-events) for
resource lifecycle (e.g. to trigger automation when a workflow is created from the dashboard), those will not fire
for the Control Plane - originated changes mentioned above. This behavior will be fixed at the latest in
the **next monthly release at the end of March**.
...
- External MCP Servers now support OAuth authentication. [Read more](/articles/mcp-servers-for-ai-agents)
- Many improvements and bug-fixes to the orchestration engine.
...
## Patch Release v2.6.4 (2026-02-27)

This release introduces polished and improvements around our [SCIM integration](/articles/scim).

Control Plane is available in Helm chart v2.329.4.

### Features and Improvements

- **Manage permissions in Testkube**: A new toggle in **Organization Management → Product features** lets admins control roles, environment access, and resource group assignments directly in the Testkube dashboard —
  while SCIM continues to handle user provisioning and team membership automatically. [Learn more about this option](/articles/scim#manage-permissions-in-testkube).
...
Control Plane is available in Helm chart v2.329.3 and Agent is available in Helm chart v2.6.2 ([Check its release notes](https://github.com/kubeshop/testkube/releases/tag/2.6.2)).
